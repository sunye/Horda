...
import org.macaw.tools.Network;
...
public class PastryPeer implements Application {
    ...
    public boolean join() throws InterruptedException, IOException {
        int freePort = Network.freeLocalPort();
    ...
    public void createPast() throws IOException {
        ...
        String storageDirectory = System.getProperty("java.io.tmpdir")
                + File.separator + "storage-" + node.getId().hashCode();

        Storage stor = new PersistentStorage(pastryIdFactory, storageDirectory, 
                4 * 1024 * 1024, node.getEnvironment());
        
        MemoryStorage mem = new MemoryStorage(pastryIdFactory);
        LRUCache cache = new LRUCache(mem, 512 * 1024, node.getEnvironment());
        StorageManagerImpl storageManager = new StorageManagerImpl(pastryIdFactory, stor, cache);
        app = new PastImpl(node, storageManager, replica.intValue(), "");
